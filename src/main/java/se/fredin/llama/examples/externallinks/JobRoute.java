...
public class JobRoute extends LlamaRoute {
    ...
    public void configure() {
        ...
        from(Endpoint.file(prop(INPUT_DIR), "items.csv"))                   // Read items.csv file from input directory
        ...
        from("seda:items-ok-split")                                              // Read the items that are sent to web based on condition in route 1
                ...
                .aggregate(constant(true), Processors::union)                    // Aggregate all the items that gets sent one by one from route1 into a collection again
        ...
        from("seda:items-nok")                                                  // Read the items that are not send to web based on condition in route 1
                ...
                .aggregate(constant(true), Processors::union)                   // Aggregate all items that gets sent one by one from route1 into a collection again
                .completionInterval(500L)                                           // Not yet sure why this is needed
...
//                        (me, ne) -> Processors.join(                                  // Call our own join class
...
//                        )
//                )
//                .process(e -> new GroupExternalLinksProcessor(e).doExecuteTask())          // Group the data calling our own group class pasing in the exchange (collection of ItemAsset beans)
//                .pollEnrich("seda:items-nok-grouped", Processors::union)     // Union our collection (that is now an Item collection) with the Item collection from the route items-nok-grouped
...
//                .startupOrder(4);                                                    // Set this route to start last
    ...
    private void filterAssets(Exchange e) {
        ...
        List<ItemAsset> assets = ProcessorUtils.<ItemAsset>asFxkBeanList(e)
                .stream()
                .filter(a -> {
                ...
                .peek(asset -> asset.setUncPath(ProcessorUtils.getTransformedUrl(asset.getUncPath(), prop("imm-url-prefix"), prop("ecom-url-prefix"))))
                .collect(Collectors.toList());
        e.getIn().setBody(assets);
    }

    private void clearPaths(Exchange exchange) {
        exchange.getIn().setBody(ProcessorUtils.<Item>asFxkBeanList(exchange)
                .stream()

...
public class EndpointTest {
    ...
    public void testSql2Params() {
        ...
        assertEquals("sql:" + SQL + "?outputClass=se.fredin.llama.bean.LlamaBean&noop=true&useIterator=false", sqlUri);
    }

    @Test
    public void testSql4Params() {
        String sqlUri = Endpoint.sql(SQL, LlamaBean.class, false, SqlResultType.ITERATE);
        assertEquals("sql:" + SQL + "?outputClass=se.fredin.llama.bean.LlamaBean&noop=false&useIterator=true", sqlUri);
    }

    @Test
    public void testSql5Params() {
        String sqlUri = Endpoint.sql(SQL, LlamaBean.class, true, SqlResultType.ITERATE, "myDataSource");
        assertEquals("sql:" + SQL + "?outputClass=se.fredin.llama.bean.LlamaBean&noop=true&useIterator=true&datasource=#myDataSource", sqlUri);
    }
